...
| `COMDLG32.OCX` file dialogs | Native chooser on Windows/macOS via `ui/file_dialog.cpp`; ImGui browser on Linux (see finding 95) |
...
| 90 | Skin colours (`frm:4648`, `modOSME.bas:833`, `skins/default/colors.ini`) | `HexToLong` turns the hex string into a VB6 `Long` that is assigned to a control's `BackColor`.  A VB6 `OLE_COLOR` is `&HBBGGRR`, so `Background=4A3C31` is RGB(0x31, 0x3C, 0x4A) -- the dark slate blue every panel has in the README screenshot -- and `LabelBack=614B3D` is RGB(0x3D, 0x4B, 0x61).  The `.frm` itself stores the same literal, `&H004A3C31&`, at every one of the ~90 `BackColor` properties | The port read the hex as RGB, giving 0x4A3C31 -- **a dark brown**.  Every panel, menu bar, title strip and status bar in the application was the wrong colour, and no skin could produce the right one because the byte order was inverted for all of them | **INCORRECT** | `parseHex` swaps the outer bytes.  The struct defaults carry the converted values with the file's spelling in a comment.  The render colours in `polyworks.ini` deliberately do **not** get this treatment: `ARGB()` (`modUtils.bas:81`) only prefixes an alpha byte before handing the value to Direct3D, which reads it as A,R,G,B, so `PointColor=CE4D4A` really is red.  The two families genuinely differ |
| 91 | `backgroundColor` / `picBackColor` (`modConfig.bas:80`, `frm:2904`, `frmPreferences.frm:2373`) | `D3DDevice.Clear` uses the `BackColor` preference, default `"555555"`, and frmPreferences exposes it as a `picBackColor` swatch | The port cleared the viewport to a hard-coded `0x2B1A0E` brown attributed to a `picMap.BackColor` that does not exist in the form, and had no such preference or control at all | **NOT PORTED** | `AppPrefs::backgroundColor` defaulting to the original's `0x555555`, persisted, used by `glClearColor`, and edited from the Preferences dialog's Colors row |
| 92 | Save-changes prompt (`frm:12752`, `frm:12772`, `frm:4387` Terminate) | VB6 calls `MsgBox` and blocks, so raising the question and acting on the answer are one statement.  No/discard proceeds with the pending action; Cancel abandons it | ImGui has no nested event loop, so the question spans frames and the pending action is parked in `pendingAfterPrompt`.  One flag, `pending`, was used both for "the box is on screen" (set when raised) and "a button was pressed" (set when answered).  `processMessageAnswer` therefore fired on the **same frame the box was raised**, with `answer == -1`, taking the continuation and dropping it down the Cancel branch before the user had seen the question.  The real button press then found nothing left to run, so **File > Exit with unsaved changes never exited** -- and New and Open behaved the same way | **BROKEN** | `MessageBox` moved to its own header with the whole contract as three operations -- `show` / `respond` / `takeAnswer` -- so the two facts cannot share a flag again.  `takeAnswer` returns false on every frame where no button has been pressed.  Covered by `TEST(message_box_answer_is_not_consumed_before_it_is_given)` |
| 93 | Scenery draw order (`frm:2955`, `frm:2992`, `frm:3039`, `frm:3041`) | Render draws the polygons in **two passes**: the background types 24/25 first, then back *and* middle scenery inside a single `scenerySprite.Begin` block, and only then every other polygon.  Front scenery is drawn later still, over the map.  So Back and Middle sit behind the level geometry and only Front is in front of it | The port drew middle scenery *after* the second polygon pass, so level 1 -- which is the default and by far the most common level -- was painted over the map.  Every scenery sprite in a normal map appeared in the foreground | **INCORRECT** | Back and Middle are both emitted between the two polygon passes, matching the single sprite block the original uses.  Verified on a generated map carrying one sprite of each level overlapping an opaque polygon: Back and Middle disappear behind it, Front does not |
| 94 | Scenery sprite dimensions (`frm:2985`, `frm:3421`, `frm:2737`, `frm:2022`) | Geometry always comes from the **texture's own** dimensions, `SceneryTextures(Style).Width/Height`, which are `imageInfo.Width/Height` of the loaded file (`frm:2147`).  The `Width`/`Height` fields in the PMS are output-only: written from the texture on save (`frm:2737`) and merely range-checked on load (`frm:2022`) | The port preferred the stored fields and fell back to the texture only when they were absent.  A map whose stored values disagree with the installed scenery pack -- a different or higher-resolution pack than the one the map was authored against -- therefore drew every sprite at the wrong aspect ratio, which on a rotated sprite reads as the rotation being wrong | **INCORRECT** | `Renderer::sceneryDrawSize` makes the texture authoritative for both the sprite and its outline, using the stored value only when the texture cannot be loaded so the outline still marks the position.  The stored fields are still written on save, preserving the round trip.  The rotation itself was confirmed correct and left alone: `GetAngle` (`modUtils.bas:174`) negates y, so a positive angle is counter-clockwise on screen, which is what `frm:3421` maps the width axis to and what the port renders |
| 95 | File chooser (`frmOpenSoldatMapEditor.frm` `CommonDialog1`, `mnuOpen_Click` frm:11570, `mnuSaveAs_Click`) | The original calls the Windows **common dialog** -- `CommonDialog1.ShowOpen` / `ShowSave` -- so opening and saving a map uses the operating system's own chooser, with its drive list, network places, filename completion and file-type filter | The ImGui port drew its own browser on every platform.  That browser has no sidebar, no recent places, no network or removable volumes, no search and no path completion, and on macOS it cannot reach anything outside the application's container because the file-access prompt is only raised by the real panel.  Choosing the Soldat directory in Preferences was worse still: a typed path with no picker at all, on the one setting the editor cannot guess | **PARTIALLY PORTED** | Native File Dialog Extended (pinned, fetched and checksummed by `cmake/NativeFileDialog.cmake`) provides the Win32 `IFileDialog` and Cocoa `NSOpenPanel`/`NSSavePanel` behind `src-cpp/ui/file_dialog.h`.  All eight file purposes go through `requestFile`, which returns its answer through the same `chosen`/`purpose` pair the browser fills, so every continuation -- notably a Save As raised to answer a discard prompt -- runs unchanged.  Preferences gains a Browse button on each directory field, using the native folder picker.  Linux keeps the in-application browser: every backend there needs GTK or a D-Bus portal, which would reintroduce the toolkit this port removed |
| 96 | Discard-changes guard (`mnuNew_Click` frm:12751, `mnuOpen_Click` frm:12769, `Terminate` frm:4387) | Each handler asks only `If prompt Then ...`.  When the map is unmodified `prompt` is False and control falls **straight through** to `NewMap` / `commonDialog.ShowOpen` / the shutdown.  The guard is a detour, never a gate: the action runs either way unless the user picks Cancel | The port's `confirmDiscardChanges` took the pending action as a continuation but *returned* `true` in the unmodified case instead of running it -- and all six call sites discarded the return value.  So on an unmodified map, which is the normal state, **File > Exit, the window close button, File > New, Open Recent and drag-and-drop all did nothing at all**.  Only the modified path worked, because that one reached the continuation through `pendingAfterPrompt`.  Reported on macOS, but not platform-specific | **BROKEN** | `confirmDiscardChanges` returns `void`, so there is no result left to ignore, and the contract is now "the continuation runs exactly once, either immediately or after the answer, unless the user cancels".  Verified by driving the real GUI under Xvfb with synthetic clicks: the close button and File > Exit both terminate an unmodified map, and on a modified one the prompt still appears and No still exits |
| 97 | `mnuOpen_Click` (frm:12769) | Opening a map is guarded by exactly the same save-changes prompt as New, in the same shape and with the same Cancel/Yes/No arms | File > Open, File > Open Compiled and Ctrl+O in the port went straight to the file chooser, so a map with unsaved edits was **replaced without any warning** | **NOT PORTED** | All three entry points route through `confirmDiscardChanges` before raising the chooser |
| 98 | List and combo controls (`frmScenery.frm` `lstScenery`, `frmPalette.frm` `cboBlend`, `frmMapSettings` combos) | VB6 `ListBox`/`ComboBox` carry their own white background with black text -- the inverse of the slate-blue form behind them -- and the selected row is white text on the highlight colour | ImGui has a single global `ImGuiCol_Text`, which the skin sets to the panels' white.  The scenery list drew **white text on the white `FrameBg`** and was legible only on the selected row; every drop-down was worse, because `ImGuiCol_PopupBg` is the dark panel colour while the combos pushed black text, giving **black on dark** in all nine lists | **INCORRECT** | `ScopedListColors` (RAII, `theme.h`) pushes the list's own text, popup, header and hot-track colours, and `listItem` inverts the text on the selected row.  It must wrap the whole `BeginCombo`/`EndCombo` pair, because the drop-down is drawn inside `EndCombo`, and must be block-scoped so it does not bleed onto the labels that follow.  Applied to all nine list and combo sites |
| 99 | Keyboard accelerators (`frm` menu `Shortcut` properties, Help.html "Keyboard shortcuts") | Every menu item carries an accelerator and the tool letters switch tools, unconditionally, except while a text box has the focus | `handleShortcuts` bailed on `io.WantCaptureKeyboard`, which sounds right but is not: with `ImGuiConfigFlags_NavEnableKeyboard` set, ImGui raises that flag for as long as `io.NavActive` is true (imgui.cpp:5018), and a tool palette holds nav focus permanently.  The flag was therefore true on essentially every frame and **not one keyboard shortcut in the application did anything** -- no Ctrl+O, Ctrl+S, Ctrl+Z, Ctrl+A, no Delete, no tool letters.  The handlers were all present and correctly wired; nothing ever called them | **BROKEN** | `io.ConfigNavCaptureKeyboard = false` at setup.  Nav still works inside dialogs, but `WantCaptureKeyboard` now means only "a widget or a modal is actually consuming keys", which is the question the guard intends to ask.  Verified against the running application: `W` switches tool, Ctrl+A then Delete clears the map, Ctrl+Z restores it, Ctrl+M opens Map Settings -- and typing `w`, `x`, `q` into the Description field inserts the letters without switching tools |
